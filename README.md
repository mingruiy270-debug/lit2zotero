...
**纳入文献不等于必须下载PDF。阅读深度由agent结合文章主题、研究问题和拟好的写作大纲自主确定，不设固定分类表。** 先理解文章要回答什么问题，再定位文献在具体章节和段落中的作用，明确拟写论点及所需细节，最后判断实际摘要是否足够，或需要核对哪些原文内容。
...
以上只是判断示例，不是“算法必须全文、生物学只需摘要”的规则。同一篇生物学论文在算法文章中可能仅作背景，在机制研究中可能是核心依据；同一篇方法论文也可能仅用于概述。大纲、文章重点或拟写论点变化后，agent应重新评估深度。

在每条证据的reason中写明“对应主题和大纲位置、支持什么论点、摘要为何足够／需要核对什么细节”。同一文献有多种用途时分别判断；其中任一用途确需全文，整篇文献仍归入“需要PDF”。有PDF不代表必须精读，无PDF也不能成为降低阅读要求的理由。
